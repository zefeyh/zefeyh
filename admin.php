...
function handleDelete(&$data) {
    if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['index'])) {
        ...
        if ($index >= 0 && $index < count($data)) {
            ...
            $fileDeleted = false;
            $fileDeleteError = '';
            if (!empty($logoPath) && strpos($logoPath, UPLOAD_DIR) === 0 && file_exists($logoPath)) {
                // 统计其他条目是否也使用此文件
                $count = 0;
                foreach ($data as $i => $entry) {
                    if ($i != $index && isset($entry['logo']) && $entry['logo'] === $logoPath) {
                        $count++;
                        break; // 只要有其他引用即停止
                    }
                }
                if ($count === 0) {
                    ...
                    if (unlink($logoPath)) {
                        ...
                        $fileDeleteError = "图标文件删除失败（权限问题）";
                    }
                } else {
                    $fileDeleteError = "图标文件被其他条目引用，未删除";
                }
            }
            ...
            if ($fileDeleted) {
                $message .= "，已删除关联图标文件";
            } elseif (!empty($fileDeleteError)) {
                $message .= "，但" . $fileDeleteError;
            }
            return $message;
        }
    }
